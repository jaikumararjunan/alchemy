...
class BacktestEngine:
    ...
    def run(
        ...
        sl_pct   = (stop_loss_pct   or getattr(self.config.trading, "stop_loss_pct",   2.0)) / 100
        tp_pct   = (take_profit_pct or getattr(self.config.trading, "take_profit_pct", 4.5)) / 100
        ...
        for i in range(warmup_bars, len(candles)):
            ...
            if open_trade is not None:
                ...
                if exit_price is not None:
                    trade = self._close_trade(open_trade, exit_price, i, exit_reason, lev)
        ...
        if open_trade is not None:
            ...
            if last_price > 0:
                trade = self._close_trade(open_trade, last_price, len(candles) - 1,
                                          "end_of_data", lev)
                balance += trade.pnl_usd
                completed.append(trade)
    ...
    def _tech_score(candles: List[Dict], price: float) -> float:
        ...
        std20 = statistics.stdev(closes[-20:]) if len(closes[-20:]) >= 2 else sma20 * 0.01
    ...
    def _open_trade(self, symbol, action, price, bar, size_usd, lev, score) -> BacktestTrade:
        ...
        return BacktestTrade(
            ...
            pnl_pct=0.0, fee_usd=fee,
            exit_reason="open", signal_score=score,
            duration_bars=0,
        )

    def _close_trade(self, t: BacktestTrade, exit_price: float,
                     bar: int, reason: str, lev: int) -> BacktestTrade:
    ...
    def _check_exits(self, t: BacktestTrade, high: float, low: float,
                     close: float, sl_pct: float, tp_pct: float):

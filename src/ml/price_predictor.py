...
class PricePredictor:
    ...
    def __init__(self):
        ...
        self._try_load()
        logger.info("PricePredictor initialised (trained=%s, n=%d)", self._is_trained, self._trained_n)
    ...
    def train(self, X: np.ndarray, y: np.ndarray) -> Dict:
        ...
        if len(X) < self.MIN_TRAIN_SAMPLES:
            logger.warning("PricePredictor: only %d samples — need %d", len(X), self.MIN_TRAIN_SAMPLES)
        ...
        result = {
            ...
            "top_features": dict(list(sorted(
                self._feature_imp.items(), key=lambda x: -x[1]
            ))[:10]),
            "elapsed_sec": round(elapsed, 2),
        }
        logger.info("PricePredictor trained: acc=%.3f±%.3f in %.1fs", result["cv_accuracy"], result["cv_std"], elapsed)
        return result
    ...
    def online_update(self, X_new: np.ndarray, y_new: np.ndarray):
        ...
        if now - self._last_retrain > self.RETRAIN_INTERVAL:
            logger.info("PricePredictor: scheduled retrain with %d new samples", len(X_new))
    ...
    def _save(self):
        try:
            import joblib
            joblib.dump({
                "rf": self._rf, "gb": self._gb, "et": self._et,
                "meta": self._meta, "scaler": self._scaler,
                "trained_n": self._trained_n, "feature_imp": self._feature_imp,
            }, self.MODEL_FILE)

    ...
    def to_dict(self) -> dict:
        return {
            ...
            "long_liquidation_levels": [
                lv.to_dict() for lv in self.long_liquidation_levels
            ],
            "short_liquidation_levels": [
                lv.to_dict() for lv in self.short_liquidation_levels
            ],
            "nearest_long_liq": self.nearest_long_liq.to_dict()
            if self.nearest_long_liq
            else None,
            "nearest_short_liq": self.nearest_short_liq.to_dict()
            if self.nearest_short_liq
            else None,
    ...
    def compute_map(self, current_price: float) -> LiquidationMap:
        ...
        for lev in self.LEVERAGE_LEVELS:
            ...
            for entry_dist_pct in [0.5, 1.5, 3.0]:
                ...
                liq_long = entry_long * (
                    1 - (1 / lev) * (1 - self.MAINTENANCE_MARGIN * lev)
                )
        ...
        long_liqs.sort(key=lambda lv: -lv.price)  # closest liq first (highest price)
        short_liqs.sort(key=lambda lv: lv.price)  # closest liq first (lowest price)

        nearest_long = next(
            (lv for lv in long_liqs if lv.severity in ("close", "moderate")), None
        )
        nearest_short = next(
            (lv for lv in short_liqs if lv.severity in ("close", "moderate")), None
        )
        ...
        if nearest_long:
            interp_parts.append(
                f"Nearest long liq at ${nearest_long.price:,.0f} "
                f"({nearest_long.distance_pct:.1f}% below)."
            )
        if nearest_short:
            interp_parts.append(
                f"Nearest short liq at ${nearest_short.price:,.0f} "
                f"({nearest_short.distance_pct:.1f}% above)."
            )
        if signal == "short_squeeze_risk":
            interp_parts.append(
                "Short positions densely stacked above — short squeeze risk."
            )
        elif signal == "long_squeeze_risk":
            interp_parts.append(
                "Long positions densely stacked below — liquidation cascade risk."
            )
        ...
        return LiquidationMap(
            ...
            or "No immediate liquidation cascade risk identified.",
        )
